...
function renderStartTimeAndTitle(
  calendarEvent: ICalendarEvent,
  keyPrefix: string
) {
  return (
    <View
      key={keyPrefix + "_" + calendarEvent.id}
      style={styles.agendaContainer}
    >
      <Text style={styles.agendaHour}>
        {" "}
        {moment(calendarEvent.startDate).format("h:mm A")}:
      </Text>
...
function AgendaAndUpcomingMeetings(props: CalenderAgendaProps) {
  const agendaComponent = props.calendarEvents.map(e =>
    renderStartTimeAndTitle(e, "agenda")
  );
  const nextMeetingComponent = props.calendarEvents
    .filter(e => moment(e.startDate) > moment(props.now).subtract(1, "hours"))
    .slice(0, 3)
    .map(nextMeeting => (
...
export default class App extends React.Component<IAppProps, IAppState> {
  ...
  async loadCalendarEvents() {
    const start = moment(this.state.now)
      .startOf("day")
      .toDate();
    const end = moment(this.state.now)
      .endOf("day")
      .toDate();
  ...
  renderWhenLoaded() {
    ...
    const cleanEvents = this.calendarHelper.stripNoisyEvents(
    ...
    return (
      <Container>
        ...
        <Content>
          <AgendaAndUpcomingMeetings
            calendarEvents={cleanEvents}
            now={moment().toDate()}
          />
        ...
        <Footer>
          <FooterTab>
            ...
            <Button
              onPress={() =>
                Linking.openURL("http://idvorkin.github.io/random")
              }
            >
              <Text>Wisdom</Text>
            </Button>
